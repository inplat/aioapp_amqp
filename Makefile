...
.PHONY: flake8
flake8: venv ## flake8
	$(VENV_BIN)/flake8 aioapp_amqp examples tests setup.py

.PHONY: bandit
bandit: venv  # find common security issues in code
	$(VENV_BIN)/bandit -r ./aioapp_amqp ./examples setup.py

.PHONY: mypy
mypy: venv ## lint
	$(VENV_BIN)/mypy aioapp_amqp examples setup.py --ignore-missing-imports
...
coverage-quiet: venv ## make coverage report
		$(VENV_BIN)/coverage run --source aioapp_amqp -m pytest
...
docs-quiet: venv ## make documentation
	rm -f docs/aioapp_amqp.rst
	rm -f docs/modules.rst
	.venv/bin/sphinx-apidoc -o docs/ aioapp_amqp
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
fast-test: venv ## fast-test
	pytest -s -v --amqp-addr=amqp://guest:guest@127.0.0.1:19821/ tests

.PHONY: fast-coverage
fast-coverage: venv ## make coverage report and open it in browser
		$(VENV_BIN)/coverage run --source aioapp_amqp -m pytest tests -v --amqp-addr=amqp://guest:guest@127.0.0.1:19821/ tests
		$(VENV_BIN)/coverage report -m
		$(VENV_BIN)/coverage html
		$(BROWSER) htmlcov/index.html
